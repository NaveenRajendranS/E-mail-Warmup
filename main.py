...
# ══════════════════════════════════════════════════════════════
    ...
    # Calculate expected Daily Target mathematically based on smart-routing
    active_senders = db.get_active_senders()
    active_senders_emails = set(s["email"].lower() for s in active_senders)
    all_active_receivers = db.get_active_receivers()
    settings = db.get_settings()
    rounds = settings.get("rounds_per_day", 5)
    batch = settings.get("batch_size", 5)
    
    daily_target = 0
    for s in active_senders:
        if "@" not in s["email"]: continue
        s_dom = s["email"].split("@")[1].lower()
        valid = [r for r in all_active_receivers if "@" in r["email"] and r["email"].split("@")[1].lower() != s_dom and r["email"].lower() != s["email"].lower()]
        
        # Internal receivers are those that are ALSO active senders (capable of replying)
        internal = [r for r in valid if r["email"].lower() in active_senders_emails]
        
        int_c = min(3, batch // 2 + 1)
        ext_c = batch - int_c
        p_int = min(len(internal), int_c)
        p_ext = min(len(valid) - len(internal), ext_c)
        tot_p = p_int + p_ext
        if tot_p < batch:
            tot_p = min(len(valid), batch)
            
        # Expected replies generated by hitting `p_int` internal senders
        daily_target += (tot_p + p_int) * rounds
    ...
    metric_cols[3].metric("🎯 Daily Goal", f"{sent_today} / {daily_target} sent")
